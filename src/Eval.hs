...
import Lang
import MonadFL
import PPrint (ppConsole, ppError)
import Subst (subst)
...
evalFuncs fns = sconcat <$> mapM go fns
 where
  ...
  go (Defined name) = lookUpFunc name >>= maybe (ppError ("Function not found: " ++ name)) evalFuncs
...
eval (V var) =
  case var of
    Global name -> lookUpExp name >>= maybe (ppError ("Variable not found: " ++ name)) eval
...
eval (App fs e t) = do
  ...
  case ty of
    Just i -> do
      ...
      e' <- eval e
      apply fs' e' i
    Nothing -> ppError ("Type not found: " ++ show t)
eval (Print e) = eval e >>= (\r -> ppConsole r >> return r)
eval (LetIn _ u v) = do
  u' <- eval u

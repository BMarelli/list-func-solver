...
instance FList CList where
  ...
  zero L l = consCL (Num 0) l
  zero R (CUnit x) = Consnoc x Empty (Num 0)
  zero R l = snocCL (Num 0) l

  succesor _ Empty = Left InvalidAplication
  succesor L (CUnit (Num x)) = Right $ CUnit (Num (succ x))
  succesor L (Consnoc (Num x) xs y) = Right $ Consnoc (Num (succ x)) xs y
  succesor R (CUnit (Num x)) = Right $ CUnit (Num (succ x))
  succesor R (Consnoc x xs (Num y)) = Right $ Consnoc x xs (Num (succ y))
  succesor _ l = Right l

  delete _ Empty = Left InvalidAplication
  delete L (CUnit (Num _)) = Right Empty
  delete L l@(Consnoc (Num _) xs y) = Right $ delHeadCL l
  delete R (CUnit (Num _)) = Right Empty
  delete R l@(Consnoc (Num _) xs y) = Right $ delLastCL l
  delete _ l = Right l
  ...
  rep fn l | lengthFL l >= 2 = rep_ (fn, []) l
    ...
    where
      ...
      rep_ ([], rst) l@(Consnoc (Num x) _ (Num y)) | x == y    = Right l

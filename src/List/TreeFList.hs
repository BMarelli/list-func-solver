...
instance FList Tree where
  ...
  zero _ Empty = Leaf 0
  zero L t = Node (lengthFL t + 1) (Leaf 0) t
  zero R t = Node (lengthFL t + 1) t (Leaf 0)

  succesor _ Empty = Left InvalidAplication
  succesor _ (Leaf x) = Right $ Leaf (succ x)
  succesor L (Node n l r) =
    let l' = succesor L l
     in Right $ Node n (fromRight l') r
  succesor R (Node n l r) =
    let r' = succesor R l
     in Right $ Node n l (fromRight r')
  succesor _ l = Right l

  delete _ Empty = Left InvalidAplication
  delete _ (Leaf (_)) = Right Empty
  delete L (Node _ l r) =
    let l' = fromRight $ delete L l
     in Right $ Node (lengthFL l' + lengthFL r) l' r
  delete R (Node _ l r) =
    let r' = fromRight $ delete R r
     in Right $ Node (lengthFL l + lengthFL r') l r'
  delete _ l = Right l
...
--     rep_ ([], rst) l@(Node _ (Leaf ( x)) (Leaf ( y))) | x == y    = Right l
--                                                             | otherwise = rep_ (reverse rst, []) l
--     rep_ ([], rst) l = Right l
--     rep_ (f:fs, []) l@(Node _ (Leaf ( x)) (Leaf ( y))) | x == y    = Right l
--                                                              | otherwise = case f of
